...
@section('css')
<link rel="stylesheet" href="{{asset('css/components/perfil/perfil.css')}}">
...
<section class="box-perfil-user">
    ...
    <form action="{{route('user.profiles.update',$user->id)}}" method="post" id="form-perfil">
      ...
      @method('PUT')
        ...
        <input type="text" name="nombres" id="nombres" class="@error('nombres') border-danger @enderror" autocomplete="off" maxlength="50" value="{{old('nombres',$user->nombres)}}">
        ...
        <div>
          ...
          <input type="text" name="apellidos" id="apellidos" class="@error('apellidos') border-danger @enderror" autocomplete="off" maxlength="50" value="{{old('apellidos',$user->apellidos)}}">
          <small class="@error('apellidos') text-danger @enderror">@error('apellidos') {{$message}} @enderror</small>
        </div>
        <div>
          <label for="usuario" class="@error('username') text-danger @enderror">Nombre de usuario *</label>
          <input type="text" name="username" id="usuario" class="@error('username') border-danger @enderror" autocomplete="off" maxlength="25" required value="{{old('username',$user->username)}}">
        ...
        <input type="email" name="email" id="correo" class="@error('email') border-danger @enderror" autocomplete="off" maxlength="100" required value="{{old('email',$user->email)}}">
          <small class="@error('email') text-danger @enderror">@error('email') {{$message}} @enderror</small>
        </div>
        <div>
          <label for="cedula" class="@error('cedula') text-danger @enderror">Numero de identificacion</label>
          <input type="number" name="cedula" id="cedula" class="@error('cedula') border-danger @enderror" autocomplete="off" maxlength="10" minlength="10" value="{{old('cedula',$user->cedula)}}">
