...
@section('section-content')
  ...
  <div class="card container-xl">
    ...
    <div class="card-body box-style-default">
      ...
      <form action="{{route('mis-actividades.update',$act->id)}}" method="post" id="mActividad">
        @csrf
        @method('PUT')

        <div class="mb-4 mt-4">
          <label for="fecha" class="form-label label-description @if ($errors->has('fecha')) text-danger @else text-secondary @endif">Fecha *</label>
          <input type="date" class="form-control @if ($errors->has('fecha')) border-danger @endif required" id="fecha" value="{{old('fecha',$act->fecha)}}" autocomplete="off" name="fecha">
        ...
        <div class="mb-4 mt-4">
          ...
          <input type="text" class="form-control @if ($errors->has('horario')) border-danger @endif" id="horario" value="{{old('horario',$act->horario)}}" maxlength="100" autocomplete="off" name="horario">
        ...
        <div class="mb-4 mt-4">
          ...
          <input type="text" class="form-control text @if ($errors->has('modalidad')) border-danger @endif" id="modalidad" value="{{old('modalidad',$act->modalidad)}}" maxlength="150" autocomplete="off" name="modalidad">
        ...
        <div class="mb-4 mt-4">
          ...
          <textarea class="form-control @error('observacion') border-danger @enderror text" id="observacion" rows="3" name="observacion" maxlength="255" style="max-height: 150px;min-height:80px;">{{old('observacion',$act->observaciones)}}</textarea>
